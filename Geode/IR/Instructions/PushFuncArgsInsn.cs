...
namespace Geode.IR.Instructions
{
    public class PushFuncArgsInsn(FunctionType type, IEnumerable<ValueRef> args) : Instruction(args)
    {
        ...
        {
            var macros = RawFunctionValue.SetArgsAndGetMacros(ctx, FuncType, [.. Arguments.Cast<ValueRef>()]);

            if (macros is LiteralValue) new StackValue(-1, ctx.Builder.RuntimeID, "macros", PrimitiveType.Compound).Store(macros, ctx);
        }
