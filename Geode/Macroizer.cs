...
namespace Geode
{
    public class Macroizer(GeodeBuilder builder)
    {
        ...
        public void Run(RenderContext ctx, IValueLike[] dependencies, Action<IConstantValue[], RenderContext> func)
        {
            ...
            {
                if (applied.TryGetValue(val, out var existing)) return existing;
        ...
        public void RunAndPropagateMacros(RenderContext ctx, IValueLike[] dependencies,
            ...
            [.. dependencies, .. ctx.Func.Decl.FuncType.MacroParameters], (args, ctx) =>
            {
                ...
                func(realArgs, [
                    ...
                    {
                        if (i.First.Type.WrapInQuotesForMacro && i.Second.Value.GetType() != typeof(NBTString)) return new(i.First.Name, new NBTString(i.Second.Value.ToString()));
