...
namespace Datapack.Net.Function.Commands
{
    public class FunctionCommand : Command
    {
        ...
        public FunctionCommand(MCFunction func, bool macro = false) : base(macro)
        {
            ...
            if (!func.Partial && func.Macro) throw new InvalidOperationException($"Function {func.ID} has macro arguments, but is not called with them");
        }
        ...
        public FunctionCommand(MCFunction func, NBTCompound arguments, bool macro = false) : base(macro)
        {
            ...
            if (!func.Partial && !func.Macro) throw new InvalidOperationException($"Function {func.ID} does not has macro arguments, but is called with them");
        }
