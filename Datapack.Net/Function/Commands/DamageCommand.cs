...
namespace Datapack.Net.Function.Commands
{
    public class DamageCommand: Command
    {
        ...
        protected override string PreBuild()
        {
            ...
            if (Position is null && By is null) return cmd.ToString();
            else if (Position is not null) cmd.Append($" at {Position}");
            else if (By is not null) cmd.Append($" by {By.Get()}{(Cause is null ? "" : $" from {Cause.Get()}")}");
