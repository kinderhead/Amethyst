...
namespace Datapack.Net.CubeLib
{
	public abstract partial class Project
	{
		...
		{
			var project = (T?)Activator.CreateInstance(typeof(T), [pack]) ?? throw new ArgumentException("Invalid project constructor");
			...
			while (FunctionsToProcess.TryDequeue(out var i))
			{
				...
				else
				{
					...
					foreach (var e in args)
					{
						...
						{
							funcArgs.Add((IRuntimeArgument?)e.GetMethod("Create")?.Invoke(null, [ArgumentStack.Dequeue()]) ?? throw new ArgumentException($"Invalid arguments for function {i.Key.Method.Name}"));
						}
					}
				...
				WithCleanup(() =>
				{
					...
					{
						if (rtp is IBaseRuntimeObject obj && !obj.Freed)
						{
							obj.FreeObj();
						}
						else
		...
		public MCFunction GuaranteeFunc(Delegate func, KeyValuePair<string, object>[] macros)
		{
			...
			if (FindFunction(func) is MCFunction mcfunc)
			{
				retFunc = mcfunc;
			}
			else
			{
				retFunc = AddFunction(func);
			}
			...
			if (uniqueMacros.Count != 0 && attr.Macros.Length == 0)
			{
				throw new ArgumentException("Attempted to call a non macro function with arguments");
			}
			else if (uniqueMacros.Count == 0 && attr.Macros.Length != 0)
			{
				throw new ArgumentException("Attempted to call a macro function without arguments");
			}
			else if (uniqueMacros.Count != attr.Macros.Length)
			{
				throw new ArgumentException("Mismatch in number of macro arguments passed to and accepted by function");
			}
			else if (uniqueMacros.Count != 0)
			{
				foreach (var i in macros)
				{
					if (!attr.Macros.Contains(i.Key))
					{
						throw new ArgumentException($"Function does not accept macro argument \"{i.Key}\"");
					}
				}
			}
		...
		{
			if (!DeclareMCAttribute.Get(func).Returns)
			{
		...
		public void CallRet(MCFunction func, ScoreRef ret, KeyValuePair<string, object>[] args, bool macro = false, int tmp = 0) => AddCommand(new Execute(macro).Store(ret).Run(BaseCall(func, args, macro, tmp)));
		...
		public FunctionCommand BaseCall(MCFunction func, KeyValuePair<string, object>[] args, bool macro = false, int tmp = 0)
		{
			...
			{
				if (NBTValue.ToNBT(i.Value) != null)
				{
					parameters[i.Key] = NBTValue.ToNBT(i.Value) ?? throw new Exception("How did we get here?");
				}
				else if (i.Value.GetType().IsAssignableTo(typeof(NBTValue)))
				{
					parameters[i.Key] = (NBTValue)i.Value;
				}
				else if (i.Value is ScoreRef score)
				{
					runtimeScores[i.Key] = score;
				}
				...
				else if (i.Value is IToPointer ptr)
				{
					pointers[i.Key] = ptr.ToPointer();
				}
				else if (i.Value is NamespacedID id)
				{
					parameters[i.Key] = id.ToString();
				}
				else if (i.Value is Storage storage)
				{
					parameters[i.Key] = storage.ToString();
				}
				else
				{
					throw new ArgumentException($"Type {i.Value.GetType().Name} is not supported yet");
				}
			}
			...
			if (Settings.VerifyMacros && !func.ID.Path.StartsWith("zz_"))
			{
				...
				if (!CurrentTarget.ID.Path.StartsWith("zz_"))
				{
					If(ret == 0, new ReturnCommand());
				}
			}
		...
		public MCFunction? FindFunction(Delegate func)
		{
			...
			{
				if (lib.BuiltOrBuilding)
				{
					return lib.FindFunction(func);
				}
		...
		public MCFunction AddFunction(Delegate func) => AddFunction(func, new(Namespace, DeclareMCAttribute.Get(func).Path));
		...
		public MCFunction AddFunction(Delegate func, NamespacedID id, bool scoped = false)
		{
			...
			if (Settings.VerifyMacros)
			{
				MacroCheckFunctions[id] = new MCFunction(new(id.Namespace, $"zz_macro_check/{id.Path}"), true);
			}
			...
			if (scoped)
			{
				...
				WithCleanup(() =>
				{
					...
					for (var i = RegistersInUse.Count - 1; i >= scope.Count; i--)
					{
		...
		public void AddCommand(Command cmd)
		{
			...
			else if (Settings.ErrorChecking)
			{
				...
				{
					if (exe.Get<Run>().Command is FunctionCommand)
					{
		...
		public void WithInit(Action func) => WithInit(() =>
													  {
														  func();
														  return false;
													  });
		...
		public MCFunction ProcessRuntimeObjectMethod(MethodInfo method, RuntimeObjectAttribute attr)
		{
			...
			if (Settings.VerifyMacros)
			{
				MacroCheckFunctions[mcFunc.ID] = new MCFunction(new(mcFunc.ID.Namespace, $"zz_macro_check/{mcFunc.ID.Path}"), true);
			}
		...
		public void PrependMain(Command cmd) => (FindFunction(Main) ?? throw new Exception("Main doesn't exist")).Prepend(cmd);
		...
		{
			if (Constants.TryGetValue(val, out var obj))
			{
				return obj;
			}
		...
		{
			if (CurrentFunctionAttrs is null || CurrentFunctionAttrs.Macros.Length == 0)
			{
				return new(mcfunc);
			}
		...
		public T AllocObj<T>(ScoreRef loc, bool rtp = true, bool cleanup = true) where T : IBaseRuntimeObject
		{
			...
			if (obj.HasMethod("init"))
			{
				CallArg(obj.GetMethod("init"), [obj]);
			}
		...
		public HeapPointer<T> Alloc<T>(ScoreRef loc, bool free = true) where T : NBTValue
		{
			...
			if (loc.Global)
			{
				Console.WriteLine("Warning: allocating using a global variable without AllocIfNull<T>()");
			}

			if (!loc.Global && free)
			{
				WithCleanup(pointer.Free);
			}
		...
		public void While(Conditional comp, Action res) => WhileTrue(() =>
																	{
		...
		public (string, string) GetLineData()
		{
			...
			{
				if (i.GetMethod()?.DeclaringType?.Namespace?.StartsWith("Datapack.Net") == true)
				{
					continue;
				}
				else if (i.GetMethod()?.DeclaringType?.BaseType != typeof(Project))
				{
					continue;
				}
				else if (i.GetFileName() is null)
				{
					continue;
				}
				else
				{
					return ($"{Path.GetFileName(i.GetFileName())}:{i.GetFileLineNumber()}", File.ReadLines(i.GetFileName() ?? throw new Exception("Couldn't read file")).Skip(i.GetFileLineNumber() - 1).Take(1).First().Trim());
				}
			}
		...
		public T EntityRef<T>(TargetSelector sel, ScoreRef var) where T : EntityWrapper
		{
			...
			if (sel.Type?.First()?.Value != entity.Type)
			{
				throw new ArgumentException("Entity target selector type is invalid");
			}
