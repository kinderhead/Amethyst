...
namespace Datapack.Net.CubeLib.EntityProperties
{
	public class EntityStructure<T>() : EntityProperty<T> where T : NBTValue
	{
		public R GetProp<R>(string path) where R : EntityProperty, new()
		{
			...
			{
				return Entity.GetAs<R>(Path + path);
			}
			else if (Value is not null)
			{
				return GetFromValue<R>(path);
			}
			else
			{
				throw new Exception("Invalid EntityStructure");
			}
		}
		...
		protected R GetFromValue<R>(string path) where R : EntityProperty, new()
		{
			...
			if (Value is NBTCompound comp)
			{
				return new() { RawValue = comp[path] };
			}
