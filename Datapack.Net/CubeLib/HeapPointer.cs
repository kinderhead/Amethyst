...
namespace Datapack.Net.CubeLib
{
	...
	public class HeapPointer<T>(MCStaticHeap heap, ScoreRef pointer, string extraPath = "") : BaseHeapPointer, IPointer<T> where T : IPointerable
	{
		...
		public override void Set(NBTValue val) => Project.ActiveProject.Std.PointerSet(StandardMacros([new("value", val.ToString())]));

		public override void Set(ScoreRef val) => Project.ActiveProject.Std.PointerSet(StandardMacros([new("value", val)]));

		public void Copy(IPointer<T> dest) => CopyUnsafe(dest);

		public override void CopyUnsafe(IStandardPointerMacros dest) => Project.ActiveProject.Std.PointerMove([.. StandardMacros(null, "2"), .. dest.StandardMacros(null, "1")]);
		...
		{
			if (typeof(T).IsAssignableTo(typeof(IBaseRuntimeObject)) && Project.Settings.ReferenceChecking)
			{
				((IBaseRuntimeObject?)Activator.CreateInstance(typeof(T), this))?.Destruct();
			}
		...
		{
			if (pointer.ExtraPath != "")
			{
				throw new Exception("Pointers with subpaths cannot be converted to a score");
			}
		...
		{
			if (NBTValue.IsNBTType<T>())
			{
				return;
			}

			if (typeof(T).IsSubclassOf(typeof(IBaseRuntimeObject)))
			{
				return;
			}

			if (typeof(T).IsSubclassOf(typeof(IRuntimeArgument)))
			{
				return;
			}
		...
		public override IPointer Cast(Type type) => (IPointer?)GetType().GetMethod("Cast", 1, [])?.MakeGenericMethod([type]).Invoke(this, []) ?? throw new Exception("Unable to cast");
	}
}
