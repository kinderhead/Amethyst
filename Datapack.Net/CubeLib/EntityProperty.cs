...
namespace Datapack.Net.CubeLib
{
	...
	public class EntityProperty<T> : EntityProperty where T : NBTValue
	{
		...
		public override void Set(Entity entity, string path)
		{
			...
			{
				entity.SetNBT(path, Pointer);
			}
			else
			{
				throw new ArgumentException("Invalid InputEntityProperty or type is not a number type");
			}
		}
		...
		public ScoreRef ToScore(ScoreRef score, double scale = 1)
		{
			if (Entity is null)
			{
				throw new ArgumentException("Can only convert an OutputEntityProperty");
			}
		...
		{
			if (Entity is null || Path is null)
			{
				throw new ArgumentException("Can only convert an OutputEntityProperty");
			}
	...
	public class FloatEntityProperty : EntityProperty<NBTFloat>
	{
		public void Set(ScoreRef val, double scale)
		{
			...
			{
				throw new ArgumentException("Invalid entity property");
			}
		}
