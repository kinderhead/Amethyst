...
namespace Datapack.Net.CubeLib.Builtins.Static
{
	public class MCStaticHeap(Storage storage, string path) : IStaticType
	{
		...
		public void Init() => Project.ActiveProject.PrependMain(new Execute().Unless.Data(Storage, Path).Run(new DataCommand.Modify(Storage, Path).Set().Value("{}")));
		...
		public void Clear() => Project.ActiveProject.AddCommand(new DataCommand.Modify(Storage, Path).Set().Value("{}"));
	}
}
