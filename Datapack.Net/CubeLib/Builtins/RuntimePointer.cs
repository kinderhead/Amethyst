...
namespace Datapack.Net.CubeLib.Builtins
{
	...
	public partial class RuntimePointer<T>(IPointer<RuntimePointer<T>> loc, string extraPath) : RuntimeObject<CubeLibStd, RuntimePointer<T>>(loc), IPointer<T> where T : IPointerable
	{
		...
		public void Set(NBTValue val) => Project.ActiveProject.Std.PointerSet(StandardMacros([new("value", val.ToString())]));

		public void Copy(IPointer<T> dest) => CopyUnsafe(dest);

		public void CopyUnsafe(IStandardPointerMacros dest) => Project.ActiveProject.Std.PointerMove([.. StandardMacros(null, "2"), .. dest.StandardMacros(null, "1")]);
		...
		public void Free()
		{
			if (Project.Settings.ReferenceChecking && IsRuntimeObject && Self is IBaseRuntimeObject obj)
			{
				obj.Destruct();
			}
		...
		public void Set(ScoreRef val) => Project.ActiveProject.Std.PointerSet(StandardMacros([new("value", val)]));
		...
		public override void Destruct() => RemoveOneReference();
