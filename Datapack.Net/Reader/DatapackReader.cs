...
namespace Datapack.Net.Reader
{
	public class DatapackReader
	{
		...
		public string ReadFile(string path)
		{
			...
			if (IsZip)
			{
				...
				using var file = zip.GetEntry(path)?.Open() ??
				                 throw new FileNotFoundException($"No resource at {path}");
		...
		public static string PathFor(NamespacedID id, string folder, string extension) =>
			Path.Combine("data", id.Namespace, folder, id.Path) + extension;
