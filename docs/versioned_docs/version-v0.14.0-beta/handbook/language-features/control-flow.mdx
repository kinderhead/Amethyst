...
:::note
`else` statements currently only work for non-forking `execute` statements. This may change in the future.
:::
...
Amethyst supports traditional C-style `for` loops as well as `continue` and `break`:
