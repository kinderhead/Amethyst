...
# Or
amethyst compile examples/test.ame -o test.zip --run
```

Once started with `amethyst run` or `amethyst compile --run`, the Minecraft server will stay alive for a specified amount of time (default 30 minutes) before automatically shutting down.
...
:::warning
Due to how Windows deals with files accessed by another program, `amethyst run` may not keep the server alive on Windows for further runs. If `amethyst run` throws an error on repeated runs, stop the `Java` process with Task Manager and report the issue. Additionally, the project maintainer no longer has access to a Windows machine so support may be limited.
:::

### Running

When using `amethyst run` or `amethyst compile --run`, any `/tellraw` commands will be logged to the console. By default, the command will never end. However it will end if it encounters a message that says "[exit]" which can be sent by calling `amethyst:exit` after your program has completed. For example:
...
The source file path is relative to the current working directory of the `amethyst compile` command when it was built. In the case of a location within the standard library, a special `@std/` prefix is used.
