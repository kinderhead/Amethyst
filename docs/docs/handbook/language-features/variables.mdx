...
* `.where(nbt predicate)`: Get a copy of a subset of the list that matches a predicate. Uses [NBT paths](https://minecraft.wiki/w/NBT_path) of the form `list[predicate]`. Only works on lists of [compounds](#objects), [maps](#maps), or [structs](./structs.mdx).
* `.all()`: Get a special reference using an [NBT path](https://minecraft.wiki/w/NBT_path) of the form `list[]`. This allows modifying the properties of all NBT compounds in a list at once.
