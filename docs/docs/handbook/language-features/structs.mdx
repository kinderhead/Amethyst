...
:::note
Properties are set to their default values if not explicitly set. Additionally, structs cannot contain strong reference properties (i.e. `int&` or [classes](./classes.mdx)) due to a lack of garbage collection capabilities.
:::
