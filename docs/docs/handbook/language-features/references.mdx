...
To reference stack variables locally, use [Weak References](#weak-references)
:::
...
References can also be checked to see if they are valid:

<Highlight code={`\
    if (ptr) { }
`}/>

The following pattern can be used to check the existence of properties of objects or elements in an array:
...
This works because while accessing a member of an object normally dereferences it, adding a [reference operator](#reference-operators) turns it back into a reference.
...
References can also be created with dedicated reference operators. This is useful for [inline commands](./inline-commands.mdx) like so:

<Highlight code={`\
    int x = 1;
    // &x also works too, but is more expensive in this context
    @/data modify $(^x) set value 4
`}/>

Dereferencing can be performed with `*` and works on all reference types.
...
References, and by extension [class](./classes.mdx) instances, can be set to null. `null` is a special reserved variable that can set any reference to null. References that are not initialized default to null. Null is essentially a random location in NBT storage. Due to a quirk with `execute unless data`, only storage locations can be used for null.
