...
Some target selector arguments, like `tag`, only accept alphanumeric strings. To support this, a special `unsafe_string` type is available that does not have any of the safeguards that normal `string`s have.
