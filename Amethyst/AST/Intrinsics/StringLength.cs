...
{
	public class StringLength(FunctionType? type = null) : Intrinsic("minecraft:string/length", type ?? new(
		FunctionModifiers.None, new VoidType(), [
		...
		]))
	{
		...
		public override ValueRef Execute(FunctionContext ctx, params ValueRef[] args) =>
			ctx.ExplicitCast(ctx.ImplicitCast(args[0], PrimitiveType.String), PrimitiveType.Int);
	}
}