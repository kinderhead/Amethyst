...
namespace Amethyst.AST.Expressions
{
    public class CallExpression(LocationRange loc, Expression func, List<Expression> args) : Expression(loc)
    {
        ...
        protected override ValueRef ExecuteImpl(FunctionContext ctx, TypeSpecifier? expected)
        {
            ...
            if (func.Value is Intrinsic i) return i.Execute(ctx, [.. newArgs.Select(i => i.Execute(ctx, null))]);
            ...
            if (func.Value is OverloadedFunctionValue overload)
            {
                ...
                func = options.Length switch
                {
                    0 => throw new NoOverloadError(overload.ID, types),
                    > 1 => throw new AmbiguousOverloadError(overload.ID, types),
                    _ => Function is PropertyExpression
                        ? ReferenceType.TryDeref(ctx.GetProperty(args[0], options[0].id.GetFile()), ctx)
            ...
            if (func.Value is FunctionValue f) return f.CallBehavior(ctx, args);
