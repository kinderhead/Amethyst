...
namespace Amethyst.Geode.IR.Instructions
{
    public class ResolveWeakRefInsn(ValueRef val) : Instruction([val])
    {
        ...
        protected override Value? ComputeReturnValue(FunctionContext ctx)
        {
            ...
            if (val is not LiteralValue l || l.Value is not NBTString ptr) throw new WeakReferenceError();
            else if (ptr.Value.Contains("stack[-1].")) return null;
            else if (ptr.Value.Contains("stack[-2].")) throw new NotImplementedException();
            else return new LiteralValue(ptr, ReturnType);
        }
    }
}
