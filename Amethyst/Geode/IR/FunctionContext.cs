...
namespace Amethyst.Geode.IR
{
	public class FunctionContext
	{
		...
		{
			if (GetMethodOrNull(val, name) is ValueRef method)
			{
				return method;
			}
			else if (val.Type.Property(name) is TypeSpecifier t)
			{
				return Add(new PropertyInsn(val, new LiteralValue(name), t));
			}
		...
		public ValueRef? GetMethodOrNull(ValueRef val, string name, TypeSpecifier? effectiveMethodType = null)
		{
			...
			if (GetGlobal($"{effectiveMethodType.ID}/{name}") is IFunctionLike func && func.FuncType.Parameters.Length >= 1)
			{
				...
				{
					return func.AsMethod(val, this);
				}
			}

			if (effectiveMethodType.BaseClass != effectiveMethodType)
			{
				return GetMethodOrNull(val, name, effectiveMethodType.BaseClass);
			}
			else
			{
				return null;
		...
		public ValueRef? ImplicitCastOrNull(ValueRef val, TypeSpecifier type)
		{
			...
			{
				return val.SetType(type); // This hopefully shouldn't cause problems with reusing values
			}
			else if (val.Type.Implements(type))
			{
				return val.SetType(type);
			}
			else if (val.Type is ReferenceTypeSpecifier r2 && r2.Inner.Implements(type))
			{
				return r2.Deref(val, this).SetType(type);
			}
			...
			else if (type.CastOverload(val, this) is ValueRef cast)
			{
