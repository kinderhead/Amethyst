...
namespace Amethyst.Geode
{
	public record class RenderContext(MCFunction MCFunction, IR.Block Block, GeodeBuilder Builder, FunctionContext Func)
	{
		...
		public Command CallSubFunction(MCFunction func)
		{
			if (Func.IsMacroFunction)
			{
				return new FunctionCommand(func, [.. Func.Decl.FuncType.Parameters.Where(i => i.Modifiers.HasFlag(AST.ParameterModifiers.Macro)).Select(i => new KeyValuePair<string, Datapack.Net.Data.NBTValue>(i.Name, $"\"$({i.Name})\""))]);
			}
			else
			{
