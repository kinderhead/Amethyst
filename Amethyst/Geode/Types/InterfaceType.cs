...
{
	public class InterfaceType(NamespacedID id, Dictionary<string, ObjectProperty> props) : TypeSpecifier
    {
		...
		protected override bool EqualsImpl(TypeSpecifier obj) => obj is InterfaceType other && Properties.Count == other.Properties.Count && Properties.All(kv => other.Properties.TryGetValue(kv.Key, out var prop) && prop.Equals(kv.Value));
        ...
        public override object Clone() => new InterfaceType(ID, new(Properties.Select(i => new KeyValuePair<string, ObjectProperty>(i.Key, new((TypeSpecifier)i.Value.Type.Clone(), i.Value.Name)))));
    }
