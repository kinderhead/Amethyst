...
namespace Amethyst.Cli
{
	...
	{
		public override int Execute(CommandContext context, ProjectInitCommandOptions settings,
			CancellationToken cancellationToken)
		{
			...
			{
				Name = settings.Name is not null
					? settings.Name
					: AnsiConsole.Ask("[aqua]Project name[/]:",
						Path.GetFileName(Path.GetFullPath(output)).ToLower()),
				Description = settings.Description is not null
					? settings.Description
					: AnsiConsole.Ask("[aqua]Data pack description[/]:", "A project made with Amethyst")
			};
