...
namespace Amethyst.Daemon
{
    public static partial class Runner
    {
        ...
        {
            if (Server.GetConfig() is null) return 1;
            ...
            {
                if (!AnsiConsole.Status().Spinner(Spinner.Known.BouncingBar).SpinnerStyle(Style.Parse("gold1"))
                                .StartAsync("[gold1]Starting Minecraft server[/]", async _ =>
                                {
                        ...
                        try
                        {
                            ...
                            if (mapLoc is null)
                            {
                                new CompilerMessager(Color.Red)
                                    .Header(
                                        $"Error loading [yellow]{lastFailedFunction}[/] ([grey underline]{srcLoc}[/])")
