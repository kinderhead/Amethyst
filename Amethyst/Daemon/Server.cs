...
namespace Amethyst.Daemon
{
    public static class Server
    {
        ...
        public static readonly string ServerFolder =
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Amethyst",
                "server");
        ...
        public static async Task<int> Setup(DaemonSetupOptions settings)
        {
            ...
            if (!await GetMinecraftServer(settings.MinecraftVersion)) return 1;

            var res = AnsiConsole.Status().Spinner(Spinner.Known.BouncingBar).SpinnerStyle(Style.Parse("gold1")).Start(
                "[gold1]Setting up Minecraft server[/]", ctx =>
                {
                    if (!StartServer(config)) return 1;
            ...
            if (res == 0) AnsiConsole.MarkupLine("[green]Complete[/]");
        ...
        public static bool StartServer(Config? config = null, bool watchOutput = false, bool timeout = false)
        {
            ...
            if (!CheckJava(config)) return false;
            ...
            new Task(async void () =>
            {
                try
                {
                    ...
                    var outputTask = Task.Run(async () =>
                    {
                        ...
                        {
                            if (line.Contains(MARKER_TEXT)) lastActiveTime = DateTime.Now;

                            if (watchOutput) AnsiConsole.MarkupLine(line.EscapeMarkup());
                        }
        ...
        public static Config? GetConfig()
        {
            if (!File.Exists(ConfigLocation) || !File.Exists(MinecraftServerLocation))
            {
                AnsiConsole.MarkupLine(
                    "[red]Minecraft server is not set up yet. Run \"amethyst setup\" to get started[/]");
                return null;
            }
        ...
        public static void UpdateServerProperties(IDictionary<string, string> props)
        {
            ...
            for (var i = 0; i < data.Length; i++)
            {
                ...
                {
                    if (data[i].StartsWith(k)) data[i] = $"{k}={v}";
                }
            }
        ...
        public static async Task<bool> GetMinecraftServer(string version)
        {
            ...
            return DownloadFiles(
                ($"Minecraft {version}",
                    new($"https://meta.fabricmc.net/v2/versions/loader/{version}/0.19.3/1.1.0/server/jar"),
                    MinecraftServerLocation),
                ("Tellraw Logger", new(tellraw), Path.Combine(ServerFolder, "mods", "tellraw-logger.jar")),
                ("Better Log4j Config",
                    new(
                        "https://github.com/BigWingBeat/better_log4j_config/releases/download/1.2.0/better_log4j_config-1.2.0-fabric.jar"),
                    Path.Combine(ServerFolder, "mods", "better-log4j-config.jar"))
            );
        }
        ...
        public static async Task<(bool, string)> GetTellrawLoggerVersion(string mcversion)
        {
            ...
            var versions = JArray.Parse(await client.GetStringAsync(
                $"https://api.modrinth.com/v2/project/tellraw-logger/version?game_versions=[\"{mcversion}\"]"));

            if (versions.Count != 0) return (true, versions[0]["files"]?[0]?["url"]?.ToString() ?? "");
        ...
        public static bool DownloadFiles(params (string name, Uri url, string path)[] files)
        {
            ...
            ).StartAsync(async ctx =>
            {
                ...
                foreach (var (name, url, path) in files)
                {
                    try
                    {
                        var task = ctx.AddTask($"Downloading {name}", false).IsIndeterminate();
                        ctx.Refresh();
                        ...
                        if (length <= 0) throw new($"Invalid content length {length}");
        ...
        public static bool CheckJava(Config config)
        {
            ...
            {
                AnsiConsole.MarkupLine(
                    "[red]Could not find Java. Make sure that the correct version of Java for the Minecraft version is selected[/]");
                return false;
            }
        }
        ...
        {
            if (File.Exists(DatapackLocation + ".zip")) File.Delete(DatapackLocation + ".zip");

            if (Directory.Exists(DatapackLocation)) Directory.Delete(DatapackLocation, true);
        }
