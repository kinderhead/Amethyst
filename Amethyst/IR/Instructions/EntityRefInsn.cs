    ...
    public class EntityRefInsn(ValueRef target) : Instruction([target])
    {
        ...
        public override void Render(RenderContext ctx) =>
            ctx.Macroize([Arg<ValueRef>(0).Expect()], (args, ctx) =>
            {
                ...
                ctx.Add(new Execute().As(target)
                                     .Run(new FunctionCommand(ctx.Func.GetGlobalOrThrow<IMinimalFunction>("amethyst:core/entity/ref").Get(new([])).ID)));
                ctx.Add(new Execute().If.Entity(target)
                                     .Run(ctx.WithFaux(ctx => ReturnValue.Expect<LValue>().Store(ctx.Func.GetFunctionReturnValue(ReturnType, -1), ctx)).Single()));
            });
